...
from spectral.config import ExecutorLLMConfig
...
class ExecutorServer:
    ...
    def _test_connection(self) -> None:
        ...
        try:
            ...
            self.client.generate(
                model=self.config.model, prompt="test", stream=False, options={"num_predict": 1}
            )
    ...
    def execute(
        ...
        try:
            response = self.client.generate(
                model=self.config.model, prompt=prompt, stream=False, options=options
            )
    ...
    def execute_stream(
        ...
        try:
            response = self.client.generate(
                model=self.config.model, prompt=prompt, stream=True, options=options
            )
    ...
    def chat(
        ...
        try:
            response = self.client.chat(
                model=self.config.model, messages=messages, stream=False, options=options
            )
    ...
    def chat_stream(
        ...
        try:
            response = self.client.chat(
                model=self.config.model, messages=messages, stream=True, options=options
            )
