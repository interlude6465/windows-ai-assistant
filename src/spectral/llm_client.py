...
class LLMClient:
    ...
    def generate_stream(
        ...
        logger.debug(
            f"Sending streaming request to {self.config.base_url} with model {self.config.model}"
        )
        logger.debug(f"Prompt length: {len(prompt)} characters")
    ...
    def chat(
        ...
        logger.debug(
            f"Sending chat request to {self.config.base_url} with model {self.config.model}"
        )
    ...
    def chat_stream(
        ...
        logger.debug(
            f"Sending streaming chat request to {self.config.base_url} with model {self.config.model}"
        )
        logger.debug(f"Messages: {len(messages)} messages")
