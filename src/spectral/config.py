...
class ExecutorLLMConfig(BaseModel):
    ...
    provider: str = Field(default="ollama", description="LLM provider (e.g., ollama, local)")
    model: str = Field(
        default="dolphin-coder:15b",
        description="Model name for execution (dolphin-coder:15b recommended)",
    )
    base_url: Optional[str] = Field(
        default="http://localhost:11434", description="Base URL for provider"
    )
...
class StorageConfig(BaseModel):
    ...
    data_dir: Path = Field(default_factory=lambda: Path.home() / ".spectral" / "data")
    logs_dir: Path = Field(default_factory=lambda: Path.home() / ".spectral" / "logs")
    config_file: Path = Field(default_factory=lambda: Path.home() / ".spectral" / "config.yaml")
...
class ConfigLoader:
    ...
    def __init__(self, config_path: Optional[Union[str, Path]] = None) -> None:
        ...
        self.config: Optional[SpectralConfig] = None

    def load(self) -> SpectralConfig:
        """
        Load configuration from file or return defaults.

        Returns:
            SpectralConfig: Loaded or default configuration
    ...
    def _load_from_file(self, path: Path) -> SpectralConfig:
        """
        ...
        Returns:
            SpectralConfig: Loaded configuration
